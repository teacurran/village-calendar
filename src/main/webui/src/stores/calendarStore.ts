...
export const useCalendarStore = defineStore('calendar', {
  ...
  actions: {
    ...
    async fetchTemplates(force: boolean = false) {
      ...
      const now = Date.now()
      if (!force && this.templates.length > 0 && this.lastFetchTime && now - this.lastFetchTime < 60000) {
        return
      }
    ...
    async updateCalendar(
      ...
      try {
        ...
        if (result.errors) {
          throw new Error(result.errors[0]?.message || 'Failed to update calendar')
        }
        ...
        if (calendar) {
          ...
          const index = this.userCalendars.findIndex((c) => c.id === calendar.id)
          if (index !== -1) {
    ...
    async deleteCalendar(calendarId: string): Promise<boolean> {
      ...
      try {
        ...
        if (result.errors) {
          throw new Error(result.errors[0]?.message || 'Failed to delete calendar')
        }

        // Remove from userCalendars list
        this.userCalendars = this.userCalendars.filter((c) => c.id !== calendarId)
  ...
  getters: {
     ...
     */
    sortedTemplates: (state) => [...state.templates].sort((a, b) => a.displayOrder - b.displayOrder),
