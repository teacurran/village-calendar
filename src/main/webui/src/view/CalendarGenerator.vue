                  ...
                  <div class="space-y-3">
                    <div class="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <div class="text-xs text-gray-600">Radius in pixels</div>
                      </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                      <div>
...
import { fetchTemplates, createTemplate, updateTemplate as updateTemplateGraphQL } from '../services/graphqlService'
  ...
  autoSaveTimeout.value = setTimeout(async () => {
    ...
    try {
      ...
      if (currentCalendarId.value) {
        ...
        const response = await sessionFetch(`/api/session-calendar/${currentCalendarId.value}/autosave`, {
          method: 'PUT',
...
const loadCalendarFromUrl = async () => {
  ...
  if (calendarId) {
    ...
    try {
      const response = await sessionFetch(`/api/session-calendar/from-template/${templateId}`, {
        method: 'POST',
      })
...
const loadTemplateConfig = async (template) => {
  if (template.configuration) {
    ...
    try {
      const response = await sessionFetch(`/api/session-calendar/from-template/${template.id}`, {
        method: 'POST',
      })
