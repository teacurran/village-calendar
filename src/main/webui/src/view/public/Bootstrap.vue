      ...
      <template #content>
        ...
        <div v-else-if="!needsBootstrap" class="info-message">
          <Message severity="info">
            ...
            <p>Admin accounts already exist in the system. Bootstrap is not needed.</p>
          </Message>
        ...
        <div v-else class="bootstrap-form">
          <Message severity="warn" :closable="false">
            ...
            <p>The system needs at least one admin account. Please select a user to promote to admin.</p>
          </Message>
          ...
          <div v-if="users.length === 0" class="no-users-message">
            <Message severity="info">
              ...
              <p>You need to authenticate with OAuth (Google or Facebook) first before creating an admin account.</p>
            </Message>
            <div class="oauth-buttons">
              <Button label="Login with Google" icon="pi pi-google" class="p-button-outlined" @click="loginWithGoogle" />
              <Button label="Login with Facebook" icon="pi pi-facebook" class="p-button-outlined" @click="loginWithFacebook" />
            </div>
          </div>
          ...
          <div v-else class="user-selection">
            ...
            <DataTable :value="users" selectionMode="single" v-model:selection="selectedUser" dataKey="id"
                       :paginator="false" class="user-table">
              <Column selectionMode="single" headerStyle="width: 3rem"></Column>
              <Column field="displayName" header="Name">
                <template #body="slotProps">
                  <div class="user-info">
                    <img v-if="slotProps.data.profileImageUrl" :src="slotProps.data.profileImageUrl"
                         class="user-avatar" alt="Profile" />
                    <div>
                      <div class="user-name">{{ slotProps.data.displayName || 'Unknown' }}</div>
...
async function createAdmin() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Failed to create admin account'
  } finally {
