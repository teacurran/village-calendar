...
/**
 * Entity representing pre-computed analytics aggregations for fast dashboard queries. Supports multi-dimensional
 * analysis with metric slicing by dimension key/value pairs.
 *
 * <p>
 * Example metrics:
 *
...
public class AnalyticsRollup extends DefaultPanacheEntityWithTimestamps {
    ...
    /**
     * Find all rollups for a specific metric, ordered by period start descending. Used for metric-specific analytics
     * queries.
     *
     * @param metricName
     *            Metric name (e.g., "page_views", "revenue", "conversions")
    ...
    /**
     * Find rollups for a specific metric and dimension key, ordered by period start descending. Used for
     * dimension-sliced metric queries.
     *
     * @param metricName
     *            Metric name
     * @param dimensionKey
     *            Dimension category (e.g., "path", "template_id", "status")
     * @return Query of analytics rollups
     */
    ...
    /**
     * Find rollups for a specific metric, dimension key, and dimension value. Used for highly specific analytics
     * queries.
     *
     * @param metricName
     *            Metric name
     ...
     *            Specific dimension value (e.g., "/templates", "&lt;uuid&gt;", "PAID")
     * @return Query of analytics rollups
     */
    public static PanacheQuery<AnalyticsRollup> findByMetricAndDimensionValue(String metricName, String dimensionKey,
            String dimensionValue) {
        return find("metricName = ?1 AND dimensionKey = ?2 AND dimensionValue = ?3 ORDER BY periodStart" + " DESC",
                metricName, dimensionKey, dimensionValue);
    }

    /**
     * Find rollups within a specific time range, ordered by period start descending. Used for time-range filtered
     * dashboard queries.
     *
     * @param since
     ...
     * @return Query of analytics rollups
     */
    public static PanacheQuery<AnalyticsRollup> findByMetricAndTimeRange(String metricName, Instant since,
            Instant until) {
        return find("metricName = ?1 AND periodStart >= ?2 AND periodEnd <= ?3 ORDER BY periodStart" + " DESC",
                metricName, since, until);
    }
    ...
    public static BigDecimal sumByMetricAndTimeRange(String metricName, Instant since, Instant until) {
        return find("SELECT COALESCE(SUM(value), 0) FROM AnalyticsRollup "
                + "WHERE metricName = ?1 AND periodStart >= ?2 AND periodEnd <= ?3", metricName, since, until)
