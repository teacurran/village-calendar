...
/**
 * Service for integrating with Stripe Checkout Sessions. Handles checkout session creation, retrieval, and webhook
 * signature validation.
 *
 * <p>
 * This service focuses on Stripe Checkout (hosted payment pages), which delegates payment form rendering and card data
 * handling to Stripe, ensuring PCI DSS compliance.
 *
 * <p>
...
public class StripeService {
     ...
     *
     * <p>
     * Returns a session with a checkout URL that redirects the customer to Stripe's hosted payment page. After payment,
     * Stripe redirects to success/cancel URLs.
     *
     * @param order
     *            The order to create a checkout session for
     * @param successUrl
    ...
    public Session createCheckoutSession(CalendarOrder order, String successUrl, String cancelUrl)
        ...
        LOG.infof("Created Stripe Checkout Session %s for order %s. Checkout URL: %s", session.getId(), order.id,
                session.getUrl());
     ...
     *
     * <p>
     * Useful for checking session status, payment status, and retrieving customer information after checkout
     * completion.
     *
     * @param sessionId
     *            Stripe Checkout Session ID (starts with "cs_")
     * @return Stripe Checkout Session
     * @throws StripeException
     *             if Stripe API call fails or session not found
     */
    public Session retrieveSession(String sessionId) throws StripeException {
        ...
        try {
            ...
            LOG.debugf("Retrieved session %s with status: %s, payment_status: %s", sessionId, session.getStatus(),
     ...
     *
     * <p>
     * Verifies that a webhook request actually came from Stripe by validating the HMAC signature in the
     * Stripe-Signature header. This prevents spoofing and ensures webhook integrity.
     *
     * <p>
     * This method can be used as a reusable validation utility, though webhook validation is already implemented in
     * WebhookResource.
     *
     * @param payload
     *            Raw webhook request body (JSON string)
    ...
    public Event validateWebhookSignature(String payload, String signatureHeader)
        ...
        try {
            ...
            LOG.debugf("Webhook signature validated successfully. Event type: %s, Event ID: %s", event.getType(),
                    event.getId());
    ...
    /**
     * Get the webhook secret for external use. Useful for services that need to validate webhooks independently.
     *
