...
public class OrderResolver {
    ...
    public CalendarOrder order(@Name("id") @Description("Order ID") @NotNull final UUID orderId) {
        ...
        if (!isAdmin && !order.user.id.equals(currentUser.id)) {
            LOG.errorf("User %s attempted to access order %s owned by user %s", currentUser.id, orderId, order.user.id);
    ...
    /**
     * Get orders for a specific user (admin only) with optional status filter. If userId is not provided, returns
    ...
    public List<CalendarOrder> orders(
            @Name("userId") @Description("User ID to fetch orders for (admin only)") final UUID userId,
        ...
        if (userId != null && !userId.equals(currentUser.id)) {
            ...
            if (!isAdmin) {
                LOG.errorf("User %s attempted to query orders for user %s without admin role", currentUser.id, userId);
    ...
    /**
     * Place an order for printed calendars. Creates order in PENDING status and returns Stripe Checkout Session for
     * payment. The order status is updated to PAID after successful payment via webhook.
     *
     * @param input
    ...
    public PaymentIntentResponse placeOrder(
        ...
        if (calendar.user == null || !calendar.user.id.equals(currentUser.id)) {
            LOG.errorf("User %s attempted to order calendar %s owned by user %s", currentUser.id, input.calendarId,
        ...
        String sessionNote = String.format("Stripe Checkout Session ID: %s%n", checkoutSession.getId());
        order.notes = order.notes == null ? sessionNote : sessionNote + order.notes;
        order.persist();

        LOG.infof("Created Stripe Checkout Session %s for order %s. Checkout URL: %s", checkoutSession.getId(),
                order.id, checkoutSession.getUrl());
    ...
    @RolesAllowed({Roles.USER, Roles.ADMIN})
    @Transactional
    public CalendarOrder cancelOrder(@Name("orderId") @Description("Order ID to cancel") @NotNull final UUID orderId,
            @Name("reason") @Description("Reason for cancellation (optional, stored in notes)") final String reason) {
        ...
        CalendarOrder cancelledOrder = orderService.cancelOrder(orderId, currentUser.id, isAdmin,
     ...
     *
     * @param orders
     *            List of orders to resolve users for
