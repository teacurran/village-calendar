...
public class CalendarGraphQL {
    ...
    /**
     * Batched field resolver for UserCalendar.user relationship. Prevents N+1 queries by
     * batch-loading users for multiple calendars. SmallRye GraphQL automatically batches field
    ...
    public List<CalendarUser> batchLoadUsers(@Source final List<UserCalendar> calendars) {
        ...
        if (userIds.isEmpty()) {
            ...
            return calendars.stream().map(c -> (CalendarUser) null).collect(Collectors.toList());
        }
        ...
        LOG.debugf("Batch loaded %d unique users for %d calendars", users.size(), calendars.size());

        return result;
    }

    /**
     * Batched field resolver for UserCalendar.template relationship. Prevents N+1 queries by
     * batch-loading templates for multiple calendars. SmallRye GraphQL automatically batches field
     * resolvers annotated with @Source when the parameter is a List.
     *
    ...
    public List<CalendarTemplate> batchLoadTemplates(@Source final List<UserCalendar> calendars) {
        ...
        List<CalendarTemplate> templates = CalendarTemplate.list("id IN ?1", templateIds);

        // Create lookup map for O(1) access
        Map<UUID, CalendarTemplate> templateMap =
                templates.stream().collect(Collectors.toMap(t -> t.id, t -> t));

        // Return templates in same order as input calendars
        // (DataLoader contract)
        List<CalendarTemplate> result =
                calendars.stream()
                        .map(c -> c.template != null ? templateMap.get(c.template.id) : null)
                        .collect(Collectors.toList());

        LOG.debugf(
                "Batch loaded %d unique templates for %d calendars",
