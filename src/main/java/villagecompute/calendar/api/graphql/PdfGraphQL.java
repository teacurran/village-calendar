...
/**
 * GraphQL resolver for PDF generation operations.
 * Handles asynchronous PDF job creation and status tracking.
 *
...
public class PdfGraphQL {
     ...
     * Users can poll this endpoint to track PDF generation progress.
     *
    ...
    /**
     * Generate PDF for a calendar asynchronously.
     * Requires authentication and calendar ownership (or calendar is public).
     * Creates a background job and returns PdfJob for status polling.
    ...
    @Description("Generate PDF for a calendar asynchronously. Creates a background job and returns PdfJob for status polling.")
    @RolesAllowed("USER")
    @Transactional
    public PdfJobStub generatePdf(
        @Name("calendarId")
        @NotNull
        @Description("Calendar ID to generate PDF for")
        String calendarId,

        @Name("watermark")
        @NotNull
        @Description("Include watermark on PDF (true for previews, false for paid downloads)")
        Boolean watermark
    ) {
        LOG.infof("Mutation generatePdf called with calendarId=%s, watermark=%s (STUB IMPLEMENTATION)",
        ...
        throw new UnsupportedOperationException(
            ...
            "TODO: Create DelayedJob for PDF rendering and return PdfJob with status tracking."
        );
    }

    /**
     * Temporary stub class representing a PDF generation job.
     * This should be replaced with the actual PdfJob entity once implemented.
     *
    ...
    public static class PdfJobStub {
        ...
        @NonNull
        @Description("Calendar being rendered to PDF")
        ...
        @NonNull
        @Description("Whether watermark should be included in PDF (for preview mode)")
