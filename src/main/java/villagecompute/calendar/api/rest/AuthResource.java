...
/**
 * REST resource for handling OAuth2 authentication flows. Provides endpoints for initiating OAuth
 * login and handling callbacks.
 */
@Path("/auth")
@Tag(
        name = "Authentication",
...
public class AuthResource {
    ...
    /**
     * Initiates OAuth login with Google. This endpoint is protected by OIDC, so accessing it will
     * trigger the OAuth flow. After successful authentication, the user is redirected to the
     * callback endpoint.
     */
    @GET
    @Path("/login/google")
    ...
    /**
     * Handles the OAuth callback from Google after successful authentication. Creates or updates
     * the user record and issues a JWT token for API access.
     *
    ...
    public Response handleGoogleCallback(@jakarta.ws.rs.QueryParam("sessionId") String sessionId) {
        ...
        try {
            ...
            CalendarUser user =
                    authenticationService.handleOAuthCallback(
                            "google", securityIdentity, sessionId);
    ...
    /**
     * Initiates OAuth login with Facebook. This endpoint is protected by OIDC, so accessing it will
     * trigger the OAuth flow. After successful authentication, the user is redirected to the
     * callback endpoint.
     */
    @GET
    ...
    @Operation(
            ...
            description =
                    "Redirects user to Facebook OAuth consent page. This endpoint triggers the"
    ...
    /**
     * Handles the OAuth callback from Facebook after successful authentication. Creates or updates
     * the user record and issues a JWT token for API access.
     *
     * @return AuthResponse with JWT token and user information
     */
    ...
    public Response handleFacebookCallback(
        ...
        try {
            ...
            CalendarUser user =
                    authenticationService.handleOAuthCallback(
                            "facebook", securityIdentity, sessionId);
    ...
    /**
     * Initiates OAuth login with Apple. This endpoint is protected by OIDC, so accessing it will
     * trigger the OAuth flow. After successful authentication, the user is redirected to the
     * callback endpoint.
     */
    @GET
    ...
    /**
     * Handles the OAuth callback from Apple after successful authentication. Creates or updates the
     * user record and issues a JWT token for API access.
     *
     * @return AuthResponse with JWT token and user information
     */
    @GET
    ...
    public Response handleAppleCallback(@jakarta.ws.rs.QueryParam("sessionId") String sessionId) {
        ...
        try {
            // Handle the OAuth callback and create/update user
            CalendarUser user =
                    authenticationService.handleOAuthCallback("apple", securityIdentity, sessionId);
    ...
    /**
     * Get current user information from JWT token. Used to verify JWT validity and get user
     * details.
     */
    @GET
    @Path("/me")
    ...
    @Operation(
            ...
            description =
                    "Returns user information based on the JWT token provided in the Authorization"
                            + " header. Used to verify token validity and retrieve current user"
                            + " details.")
