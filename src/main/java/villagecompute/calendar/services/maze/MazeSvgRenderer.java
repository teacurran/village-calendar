...
public class MazeSvgRenderer {
    ...
    /** Format prefix for the opening SVG tag with xmlns and viewBox attributes */
    private static final String SVG_OPEN_PREFIX = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 %d %d\"";

    /** Format prefix for circle markers (used for start/end markers) */
    private static final String SVG_CIRCLE_PREFIX = "  <circle cx=\"%d\" cy=\"%d\" r=\"%d\" fill=\"%s\"";

    /** Format prefix for hexagonal wall lines using floating-point coordinates */
    private static final String SVG_HEX_LINE_PREFIX = "    <line x1=\"%.1f\" y1=\"%.1f\" x2=\"%.1f\" y2=\"%.1f\"";

    /** Format tail with stroke color and width attributes that closes a self-closing element */
    ...
    private String renderOrthogonal() {
        ...
        svg.append(String.format(SVG_OPEN_PREFIX + " width=\"%d\" height=\"%d\">%n", PAGE_WIDTH, PAGE_HEIGHT,
        ...
        svg.append(String.format(SVG_CIRCLE_PREFIX + " class=\"start-marker\"/>%n", startX, startY, markerRadius,
                DEFAULT_START_COLOR));

        // Draw end marker
        int endX = offsetX + grid.getEndX() * cellSize + cellSize / 2;
        int endY = offsetY + grid.getEndY() * cellSize + cellSize / 2;
        svg.append(String.format(SVG_CIRCLE_PREFIX + " class=\"end-marker\"/>%n", endX, endY, markerRadius,
                DEFAULT_END_COLOR));
    ...
    private void drawHexWalls(StringBuilder svg, MazeCell cell, int x, int y, double[] center, double hexSize,
        ...
        if (cell.northEastWall && hasNENeighbor(x, y, gridWidth, gridHeight, evenRow)) {
            svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[0][0], vertices[0][1],
                    vertices[1][0], vertices[1][1], innerWallColor, INNER_WALL_THICKNESS));
        }

        // E wall: vertices 1-2 (if wall exists and not at right edge)
        if (cell.eastWall && x < gridWidth - 1) {
            svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[1][0], vertices[1][1],
                    vertices[2][0], vertices[2][1], innerWallColor, INNER_WALL_THICKNESS));
        }

        // SE wall: vertices 2-3 (if wall exists and not at edge)
        if (cell.southEastWall && hasSENeighbor(x, y, gridWidth, gridHeight, evenRow)) {
            svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[2][0], vertices[2][1],
                    vertices[3][0], vertices[3][1], innerWallColor, INNER_WALL_THICKNESS));
        }
        ...
        for (int y = 0; y < gridHeight; y++) {
            for (int x = 0; x < gridWidth; x++) {
                ...
                if (y == 0) {
                    ...
                    if (evenRow || x == 0) {
                        svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[5][0],
                                vertices[5][1], vertices[0][0], vertices[0][1], outerWallColor, OUTER_WALL_THICKNESS));
                    }
                    // Top-right wall (vertices 0-1)
                    if (evenRow || x == gridWidth - 1) {
                        svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[0][0],
                                vertices[0][1], vertices[1][0], vertices[1][1], outerWallColor, OUTER_WALL_THICKNESS));
                    }
                }
                ...
                if (y == gridHeight - 1) {
                    ...
                    if (evenRow || x == 0) {
                        svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[4][0],
                                vertices[4][1], vertices[3][0], vertices[3][1], outerWallColor, OUTER_WALL_THICKNESS));
                    }
                    // Bottom-right wall (vertices 3-2)
                    if (evenRow || x == gridWidth - 1) {
                        svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[3][0],
                                vertices[3][1], vertices[2][0], vertices[2][1], outerWallColor, OUTER_WALL_THICKNESS));
                    }
                }
                ...
                if (x == 0) {
                    svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[4][0],
                            vertices[4][1], vertices[5][0], vertices[5][1], outerWallColor, OUTER_WALL_THICKNESS));
                }

                // Right edge (x == gridWidth - 1): draw right wall (vertices 1-2)
                if (x == gridWidth - 1) {
                    svg.append(String.format(SVG_HEX_LINE_PREFIX + SVG_STROKE_ATTRS_TAIL, vertices[1][0],
                            vertices[1][1], vertices[2][0], vertices[2][1], outerWallColor, OUTER_WALL_THICKNESS));
                }
            }
        }
    }
    ...
    private String renderTheta() {
        ...
        // Start marker (center)
        svg.append(String.format(SVG_CIRCLE_PREFIX + " class=\"start-marker\"/>%n", centerX, centerY, markerRadius,
                DEFAULT_START_COLOR));

        // End marker (edge)
        svg.append(String.format(SVG_CIRCLE_PREFIX + " class=\"end-marker\"/>%n", centerX + maxRadius - ringSpacing / 2,
                centerY, markerRadius, DEFAULT_END_COLOR));
