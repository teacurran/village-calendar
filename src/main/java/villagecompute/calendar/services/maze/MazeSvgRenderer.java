...
/**
 * Renders a MazeGrid to SVG format.
 * Designed for 35" x 23" page with 1" margins on all sides.
 */
public class MazeSvgRenderer {
    ...
    private String renderOrthogonal() {
        ...
        if (showSolution && grid.getSolutionPath() != null) {
            ...
            for (int i = 0; i < path.size() - 1; i++) {
                ...
                svg.append(String.format(
                    "    <line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" stroke=\"%s\" stroke-width=\"%d\" stroke-linecap=\"round\" opacity=\"0.6\"/>\n",
        ...
        for (int x = 0; x < gridWidth; x++) {
            for (int y = 0; y < gridHeight; y++) {
                ...
                if (cell.eastWall && x < gridWidth - 1) {
                    svg.append(String.format(
                        "    <line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" stroke=\"%s\" stroke-width=\"%d\" stroke-linecap=\"square\"/>\n",
        ...
        svg.append(String.format(
            "  <circle cx=\"%d\" cy=\"%d\" r=\"%d\" fill=\"%s\" class=\"start-marker\"/>\n",
        ...
        svg.append(String.format(
            "  <circle cx=\"%d\" cy=\"%d\" r=\"%d\" fill=\"%s\" class=\"end-marker\"/>\n",
    ...
    private String renderDelta() {
        ...
        return renderOrthogonal().replace("<svg", "<!-- Delta maze rendering coming soon -->\n<svg");
    }

    private String renderSigma() {
        // Hexagonal maze rendering - for now, use orthogonal
        return renderOrthogonal().replace("<svg", "<!-- Sigma maze rendering coming soon -->\n<svg");
    }
