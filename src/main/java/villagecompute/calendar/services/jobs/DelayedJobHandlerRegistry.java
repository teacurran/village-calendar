...
/**
 * Auto-discovers and registers all DelayedJobHandler implementations.
 * Uses CDI Instance to find all beans implementing DelayedJobHandler.
 */
@ApplicationScoped
@Startup
        ...
        for (DelayedJobHandler handler : allHandlers) {
            ...
            if (handlersByQueue.containsKey(queueName)) {
                throw new IllegalStateException(
                    "Duplicate queue name '" + queueName + "' for handlers: " +
                    handlersByQueue.get(queueName).getClass().getName() + " and " +
            ...
            LOG.infof("Registered handler: %s (queue=%s, priority=%d)",
