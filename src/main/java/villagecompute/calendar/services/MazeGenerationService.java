...
/**
 * Service for generating mazes and their SVG representations. Size (1-20) controls the number of
 * cells. Difficulty (1-20) controls the solution path length.
 */
@ApplicationScoped
public class MazeGenerationService {
    ...
    /**
     * Convert size (1-20) to grid dimensions. Size 1 = ~15 cells wide (simple maze, ~2.2" paths)
     * Size 20 = ~132 cells wide (complex maze, ~0.25" / 1/4" paths) Height is calculated to
    ...
    /**
     * Generate a maze and return the SVG representation. The maze fills the 35"x23" page with 1"
    ...
    /**
     * Generate a maze for a UserMaze entity and update it with the SVG and solution. The maze fills
     * the 35"x23" page with 1" margins on all sides.
     */
    public void generateAndUpdateMaze(UserMaze maze) {
        ...
        MazeSvgRenderer renderer =
                new MazeSvgRenderer(grid, innerWallColor, outerWallColor, pathColor, showSolution);
    ...
    /**
     * Generate a preview SVG for given parameters (without persisting). Uses a fixed seed for
     * consistent preview.
     */
    public String generatePreview(
            MazeType type, int size, int difficulty, boolean showSolution, boolean showDeadEnds) {
        return generateMazeSvg(
                type,
                size,
                difficulty,
