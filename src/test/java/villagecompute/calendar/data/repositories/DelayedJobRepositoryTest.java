...
class DelayedJobRepositoryTest {
    ...
    void testFindById() {
        // Given
        DelayedJob job = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-123",
        ...
        assertEquals(DelayedJobQueue.EMAIL_GENERAL, found.get().queue);
    }
    ...
    void testFindReadyToRun() {
        ...
        DelayedJob ready1 = createJob(DelayedJobQueue.EMAIL_ORDER_CONFIRMATION, "actor-1",
            now.minus(10, ChronoUnit.MINUTES));
        ready1.priority = 10;
        repository.persist(ready1);

        DelayedJob ready2 = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-2",
            now.minus(5, ChronoUnit.MINUTES));
        ready2.priority = 5;
        repository.persist(ready2);

        // Not ready (runAt in future)
        DelayedJob notReady = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-3",
            now.plus(10, ChronoUnit.MINUTES));
        repository.persist(notReady);

        // Already complete
        DelayedJob complete = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-4",
            now.minus(20, ChronoUnit.MINUTES));
        complete.complete = true;
        repository.persist(complete);

        // Locked
        DelayedJob locked = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-5",
        ...
        assertTrue(readyJobs.stream().allMatch(j ->
            !j.runAt.isAfter(Instant.now()) && !j.complete && !j.locked));
    ...
    void testFindReadyToRun_RespectsLimit() {
        ...
        // Create 5 ready jobs
        for (int i = 0; i < 5; i++) {
            DelayedJob job = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-" + i,
    ...
    @Test
    @Transactional
    void testFindByQueue() {
        // Given
        repository.persist(createJobWithPriority(DelayedJobQueue.EMAIL_ORDER_CONFIRMATION, "actor-1", 10,
            Instant.now().plus(5, ChronoUnit.MINUTES)));
        repository.persist(createJobWithPriority(DelayedJobQueue.EMAIL_ORDER_CONFIRMATION, "actor-2", 5,
            Instant.now().plus(10, ChronoUnit.MINUTES)));
        repository.persist(createJobWithPriority(DelayedJobQueue.EMAIL_GENERAL, "actor-3", 5,
            Instant.now().plus(1, ChronoUnit.MINUTES)));
        repository.persist(createJobWithPriority(DelayedJobQueue.EMAIL_SHIPPING_NOTIFICATION, "actor-4", 10,
            Instant.now().plus(2, ChronoUnit.MINUTES)));
        entityManager.flush();

        // When
        List<DelayedJob> orderConfirmationJobs = repository.findByQueue(DelayedJobQueue.EMAIL_ORDER_CONFIRMATION);

        // Then
        assertEquals(2, orderConfirmationJobs.size());
        assertTrue(orderConfirmationJobs.stream().allMatch(j ->
            j.queue == DelayedJobQueue.EMAIL_ORDER_CONFIRMATION));
        // Verify ORDER BY priority DESC, runAt ASC
    ...
    void testFindByActorId() {
        ...
        DelayedJob job1 = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-123",
            now.minus(10, ChronoUnit.MINUTES));
        ...
        DelayedJob job2 = createJob(DelayedJobQueue.EMAIL_ORDER_CONFIRMATION, "actor-123",
            now.minus(5, ChronoUnit.MINUTES));
        repository.persist(job2);

        DelayedJob job3 = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-456",
    ...
    void testFindIncomplete() {
        // Given
        DelayedJob incomplete1 = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-1",
            Instant.now().plus(1, ChronoUnit.MINUTES));
        incomplete1.complete = false;
        incomplete1.priority = 10;
        repository.persist(incomplete1);

        DelayedJob incomplete2 = createJob(DelayedJobQueue.EMAIL_ORDER_CONFIRMATION, "actor-2",
            Instant.now().plus(5, ChronoUnit.MINUTES));
        incomplete2.complete = false;
        incomplete2.priority = 5;
        repository.persist(incomplete2);

        DelayedJob complete = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-3",
            Instant.now().plus(2, ChronoUnit.MINUTES));
        complete.complete = true;
    ...
    void testFindFailed() {
        ...
        DelayedJob failed1 = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-1",
            now.minus(10, ChronoUnit.MINUTES));
        ...
        DelayedJob failed2 = createJob(DelayedJobQueue.EMAIL_ORDER_CONFIRMATION, "actor-2",
            now.minus(20, ChronoUnit.MINUTES));
        ...
        DelayedJob successful = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-3",
            now.minus(15, ChronoUnit.MINUTES));
    ...
    void testPersist() {
        // Given
        DelayedJob job = createJob(DelayedJobQueue.EMAIL_SHIPPING_NOTIFICATION, "order-456",
    ...
    void testListAll() {
        // Given
        repository.persist(createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-1", Instant.now()));
        repository.persist(createJob(DelayedJobQueue.EMAIL_ORDER_CONFIRMATION, "actor-2", Instant.now()));
        repository.persist(createJob(DelayedJobQueue.EMAIL_SHIPPING_NOTIFICATION, "actor-3", Instant.now()));
        entityManager.flush();
    ...
    void testDelete() {
        // Given
        DelayedJob job = createJob(DelayedJobQueue.EMAIL_GENERAL, "to-delete", Instant.now());
        repository.persist(job);
        entityManager.flush();
    ...
    void testJobUnlock() {
        // Given
        DelayedJob job = createJob(DelayedJobQueue.EMAIL_GENERAL, "actor-1", Instant.now());
        job.locked = true;
    ...
    private DelayedJob createJob(DelayedJobQueue queue, String actorId, Instant runAt) {
        DelayedJob job = new DelayedJob();
        job.queue = queue;
        job.actorId = actorId;
        job.runAt = runAt;
        job.priority = queue.getPriority();
        job.attempts = 0;
        job.complete = false;
        job.locked = false;
    ...
    private DelayedJob createJobWithPriority(DelayedJobQueue queue, String actorId, int priority, Instant runAt) {
        DelayedJob job = createJob(queue, actorId, runAt);
