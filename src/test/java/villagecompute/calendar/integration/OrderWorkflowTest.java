...
/**
 * Integration tests for order placement and payment workflow. Tests the complete e-commerce flow
 * from order creation to payment webhook processing.
 *
 * <p>This test suite validates: 1. Order creation via GraphQL with Stripe PaymentIntent 2. Webhook
 * payment success processing (checkout.session.completed) 3. Email job enqueueing on payment
 * success 4. Idempotent webhook processing (duplicate webhooks handled correctly)
 */
@QuarkusTest
...
class OrderWorkflowTest {
    ...
    void testCreateOrder_ServiceLayer_ReturnsPendingOrder() throws Exception {
        ...
        assertEquals(
                CalendarOrder.STATUS_PENDING, order.status, "Order should be in PENDING status");
        ...
        assertTrue(
                order.totalPrice.compareTo(BigDecimal.ZERO) > 0, "Total price should be positive");
    ...
    void testWebhookPaymentSuccess_UpdatesOrderToPaid() throws Exception {
        // Given: Create an order in PENDING status (use programmatic transaction to commit before
        // HTTP request)
        CalendarOrder order =
    ...
    void testCreateOrder_CalculatesTotalWithShipping() throws Exception {
        ...
        assertTrue(
                order.totalPrice.compareTo(order.unitPrice.multiply(BigDecimal.valueOf(2))) >= 0,
    ...
    void testOrderItem_YearStoredInConfiguration() throws Exception {
        ...
        assertEquals(
                2026,
                savedItem.configuration.get("year").asInt(),
                "Configuration year should be 2026");
    }
