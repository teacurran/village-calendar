...
/**
 * Integration tests for order placement and payment workflow. Tests the complete e-commerce flow from order creation to
 * payment webhook processing.
 *
 * <p>
 * This test suite validates: 1. Order creation via GraphQL with Stripe PaymentIntent 2. Webhook payment success
 * processing (checkout.session.completed) 3. Email job enqueueing on payment success 4. Idempotent webhook processing
...
class OrderWorkflowTest {
    ...
    void testCreateOrder_ServiceLayer_ReturnsPendingOrder() throws Exception {
        ...
        CalendarOrder order = orderService.createOrder(testUser, testCalendar, 1, // quantity
    ...
    void testWebhookPaymentSuccess_UpdatesOrderToPaid() throws Exception {
        ...
        given().contentType(ContentType.JSON).header(HEADER_STRIPE_SIGNATURE, signature).body(webhookPayload).when()
                .post("/api/webhooks/stripe").then().statusCode(200).body("status", equalTo("success"));
    ...
    void testWebhookPaymentSuccess_EnqueuesEmailJob() throws Exception {
        ...
        assertEquals("OrderEmailJobHandler", jobs.get(0).queueName, "Email job should be in ORDER_CONFIRMATION queue");
    ...
    void testCreateOrder_CalculatesTotalWithShipping() throws Exception {
        ...
        CalendarOrder order = orderService.createOrder(testUser, testCalendar, 2, // quantity
                new BigDecimal("25.00"), // unitPrice
                portlandAddress);
