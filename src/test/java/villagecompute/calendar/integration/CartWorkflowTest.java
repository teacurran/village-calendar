 ...
 *
 * <p>
 * Tests: 1. Guest adds item to cart via GraphQL with X-Session-ID header 2. Same session ID returns same cart with
 * items 3. Different session IDs have isolated carts 4. Static page add-to-cart workflow (no templateId) 5. Cart
 * persists across multiple requests
 *
 * <p>
 * These tests verify the critical flow where static product pages add items to cart and redirect to checkout.
 */
@QuarkusTest
...
public class CartWorkflowTest {
    ...
    void testAddToCart_WithSessionIdHeader_CreatesCartItem() {
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body("""
                {
                    "query": "%s",
                    "variables": %s
                }
                """.formatted(mutation.replace("\n", "\\n").replace("\"", "\\\""), variables)).when().post("/graphql")
                .then().statusCode(200).body("data.addToCart.itemCount", equalTo(1))
    ...
    void testGetCart_SameSessionId_ReturnsSameCart() {
        ...
        String cartId = given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(addMutation)
                .when().post("/graphql").then().statusCode(200).body("data.addToCart.itemCount", equalTo(1)).extract()
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(getQuery).when()
                .post("/graphql").then().statusCode(200).body("data.cart.id", equalTo(cartId))
    ...
    void testCartIsolation_DifferentSessionIds_SeparateCarts() {
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId1).body(addMutation1).when()
                .post("/graphql").then().statusCode(200).body("data.addToCart.itemCount", equalTo(1));
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId2).body(getQuery).when()
                .post("/graphql").then().statusCode(200).body("data.cart.itemCount", equalTo(0));
    }
    ...
    void testStaticPageAddToCart_NoTemplateId_WorksCorrectly() {
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(addMutation).when()
                .post("/graphql").then().statusCode(200).body("data.addToCart.itemCount", equalTo(1))
                .body("data.addToCart.items[0].generatorType", equalTo("calendar"))
    ...
    void testCheckoutFlow_CartPersistsAcrossRequests() {
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(getQuery).when()
                .post("/graphql").then().statusCode(200).body("data.cart.itemCount", equalTo(1))
    ...
    void testAddMultipleItems_DifferentConfigurations_CreatesSeparateLineItems() {
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(addMutation1).when()
                .post("/graphql").then().statusCode(200).body("data.addToCart.itemCount", equalTo(1));
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(addMutation2).when()
                .post("/graphql").then().statusCode(200).body("data.addToCart.itemCount", equalTo(3)) // 1 + 2 = 3 total
                                                                                                      // quantity
    ...
    void testClearCart_RemovesAllItems() {
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(addMutation).when()
                .post("/graphql").then().statusCode(200).body("data.addToCart.itemCount", equalTo(3));
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(clearMutation).when()
                .post("/graphql").then().statusCode(200).body("data.clearCart.itemCount", equalTo(0))
    ...
    void testDatabasePersistence_CartSavedCorrectly() {
        ...
        given().contentType(ContentType.JSON).header(HEADER_X_SESSION_ID, sessionId).body(addMutation).when()
                .post("/graphql").then().statusCode(200);
