...
/** Tests for SessionCalendarResource. Tests session-based calendar management endpoints. */
@QuarkusTest
class SessionCalendarResourceTest {
    ...
    void setUp() {
        ...
        QuarkusTransaction.requiringNew().run(() -> {
            ...
            testTemplate.configuration = objectMapper.createObjectNode().put("theme", "modern").put("year", 2025);
    ...
    void testGetCurrentCalendar_WithNoSession_ReturnsBadRequest() {
        given().accept(ContentType.JSON).when().get("/api/session-calendar/current").then().statusCode(400)
                .body("error", equalTo("No session found"));
    }

    @Test
    void testGetCurrentCalendar_WithEmptySession_ReturnsBadRequest() {
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, "").when().get("/api/session-calendar/current")
                .then().statusCode(400).body("error", equalTo("No session found"));
    }

    @Test
    void testGetCurrentCalendar_WithNoExistingCalendar_ReturnsNotFound() {
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, testSessionId).when()
                .get("/api/session-calendar/current").then().statusCode(404)
    ...
    void testGetCurrentCalendar_WithExistingCalendar_ReturnsCalendar() {
        ...
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, testSessionId).when()
                .get("/api/session-calendar/current").then().statusCode(200).body("id", equalTo(calendarId.toString()))
                .body("name", equalTo("Test Current Calendar")).body("sessionId", equalTo(testSessionId));
    }
    ...
    void testSaveSessionCalendar_WithNoSession_ReturnsBadRequest() {
        given().contentType(ContentType.JSON).body("{\"name\": \"Test Calendar\"}").when()
                .post("/api/session-calendar/save").then().statusCode(400).body("error", equalTo("No session found"));
    }

    @Test
    void testSaveSessionCalendar_CreatesNewCalendar() {
        ObjectNode request = objectMapper.createObjectNode().put("name", "My New Calendar");
        request.set("configuration", objectMapper.createObjectNode().put("theme", "modern").put("year", 2025));

        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).body(request.toString()).when()
                .post("/api/session-calendar/save").then().statusCode(200).body("id", notNullValue())
                .body("sessionId", equalTo(testSessionId));

        // Verify calendar was created
        UserCalendar calendar = QuarkusTransaction.requiringNew()
    ...
    void testSaveSessionCalendar_UpdatesExistingCalendar() {
        ...
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).body(request.toString()).when()
                .post("/api/session-calendar/save").then().statusCode(200);
    ...
    void testCreateFromTemplate_WithNoSession_ReturnsBadRequest() {
        given().contentType(ContentType.JSON).when().post("/api/session-calendar/from-template/" + UUID.randomUUID())
                .then().statusCode(400).body("error", equalTo("No session found"));
    }
    ...
    void testCreateFromTemplate_WithInvalidTemplateId_ReturnsNotFound() {
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).when()
                .post("/api/session-calendar/from-template/" + UUID.randomUUID()).then().statusCode(404)
    ...
    void testCreateFromTemplate_WithInactiveTemplate_ReturnsNotFound() {
        ...
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).when()
                .post("/api/session-calendar/from-template/" + inactiveTemplateId).then().statusCode(404)
                .body("error", equalTo("Template not found"));
    }
    ...
    void testCreateFromTemplate_CreatesCalendarFromTemplate() {
        ...
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).when()
                .post("/api/session-calendar/from-template/" + templateId).then().statusCode(200)
                .body("id", notNullValue()).body("configuration", notNullValue());
    ...
    void testGetCalendar_WithNonexistentId_ReturnsNotFound() {
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, testSessionId).when()
                .get("/api/session-calendar/" + UUID.randomUUID()).then().statusCode(404)
    ...
    void testGetCalendar_OwnPublicCalendar_ReturnsWithIsOwnCalendarTrue() {
        ...
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, testSessionId).when()
                .get("/api/session-calendar/" + calendarId).then().statusCode(200)
                .body("calendar.id", equalTo(calendarId.toString()))
                .body("calendar.name", equalTo("My Public Calendar")).body("isOwnCalendar", equalTo(true));
    }
    ...
    void testGetCalendar_OtherUserPublicCalendar_ReturnsWithIsOwnCalendarFalse() {
        ...
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, testSessionId).when()
                .get("/api/session-calendar/" + calendarId).then().statusCode(200)
                .body("calendar.id", equalTo(calendarId.toString())).body("isOwnCalendar", equalTo(false));
    }
    ...
    void testGetCalendar_OtherUserPrivateCalendar_ReturnsForbidden() {
        ...
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, testSessionId).when()
                .get("/api/session-calendar/" + calendarId).then().statusCode(403)
    ...
    void testGetCalendar_OwnPrivateCalendar_ReturnsSuccessfully() {
        ...
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, testSessionId).when()
                .get("/api/session-calendar/" + calendarId).then().statusCode(200)
                .body("calendar.id", equalTo(calendarId.toString())).body("isOwnCalendar", equalTo(true));
    }
    ...
    void testGetCalendar_WithNoSessionHeader_CanViewPublicCalendar() {
        ...
        given().accept(ContentType.JSON).when().get("/api/session-calendar/" + calendarId).then().statusCode(200)
    ...
    void testAutosaveCalendar_WithNoSession_ReturnsBadRequest() {
        given().contentType(ContentType.JSON).body("{\"name\": \"Test\"}").when()
                .put("/api/session-calendar/" + UUID.randomUUID() + "/autosave").then().statusCode(400)
                .body("error", equalTo("No session found"));
    }

    @Test
    void testAutosaveCalendar_WithNonexistentCalendar_ReturnsNotFound() {
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).body("{\"name\": \"Test\"}")
                .when().put("/api/session-calendar/" + UUID.randomUUID() + "/autosave").then().statusCode(404)
                .body("error", equalTo("Calendar not found"));
    }
    ...
    void testAutosaveCalendar_WithOtherSessionCalendar_ReturnsForbidden() {
        ...
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).body("{\"name\": \"Hacked\"}")
                .when().put("/api/session-calendar/" + calendarId + "/autosave").then().statusCode(403)
    ...
    void testAutosaveCalendar_UpdatesConfiguration() {
        ...
        ObjectNode request = objectMapper.createObjectNode().put("name", "Updated Name");
        request.set("configuration", objectMapper.createObjectNode().put("theme", "new").put("year", 2025));

        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).body(request.toString()).when()
                .put("/api/session-calendar/" + calendarId + "/autosave").then().statusCode(200)
                .body("success", equalTo(true)).body("id", equalTo(calendarId.toString())).body("svg", notNullValue());

        // Verify updates
        UserCalendar calendar = QuarkusTransaction.requiringNew().call(() -> UserCalendar.findById(calendarId));
        assertEquals("Updated Name", calendar.name);
        assertEquals(2025, calendar.year);
    ...
    void testCreateNewCalendar_WithNoSession_ReturnsBadRequest() {
        given().contentType(ContentType.JSON).when().post("/api/session-calendar/new").then().statusCode(400)
                .body("error", equalTo("No session found"));
    }
    ...
    void testCreateNewCalendar_CreatesCalendarWithDefaults() {
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).when()
                .post("/api/session-calendar/new").then().statusCode(200).body("id", notNullValue())
                .body("configuration", notNullValue()).body("svg", notNullValue()).body("existing", equalTo(false));
    ...
    void testCreateNewCalendar_ReturnsExistingCalendarIfPresent() {
        ...
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).when()
                .post("/api/session-calendar/new").then().statusCode(200).body("id", equalTo(existingId.toString()))
                .body("existing", equalTo(true)).body("svg", equalTo("<svg>existing</svg>"));
    }
    ...
    void testCreateNewCalendar_UsesDefaultTemplateIfAvailable() {
        ...
        QuarkusTransaction.requiringNew().run(() -> {
            ...
            defaultTemplate.configuration = objectMapper.createObjectNode().put("theme", "from-default-template")
                    .put("year", 2026);
            defaultTemplate.persist();
        });

        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).when()
                .post("/api/session-calendar/new").then().statusCode(200).body("existing", equalTo(false));
    ...
    void testCreateNewCalendar_WithInactiveDefaultTemplate_UsesHardcodedDefaults() {
        ...
        given().header(HEADER_X_SESSION_ID, testSessionId).contentType(ContentType.JSON).when()
                .post("/api/session-calendar/new").then().statusCode(200);
    ...
    void testSessionCalendarDTO_IncludesAllFields() {
        UUID calendarId = QuarkusTransaction.requiringNew().call(() -> {
            CalendarTemplate template = CalendarTemplate.<CalendarTemplate>find("slug", "test-session-template")
                    .firstResult();
        ...
        given().accept(ContentType.JSON).header(HEADER_X_SESSION_ID, testSessionId).when()
                .get("/api/session-calendar/" + calendarId).then().statusCode(200)
                .body("calendar.id", equalTo(calendarId.toString())).body("calendar.name", equalTo("Full Calendar"))
                .body("calendar.sessionId", equalTo(testSessionId)).body("calendar.isPublic", equalTo(true))
                .body("calendar.templateId", notNullValue()).body("calendar.configuration.theme", equalTo("complete"));
    }
