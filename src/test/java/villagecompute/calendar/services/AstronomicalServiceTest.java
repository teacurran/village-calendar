...
/**
 * Unit tests for AstronomicalCalculationService. Verifies moon phase calculations, Hebrew calendar conversions, and
...
public class AstronomicalServiceTest {
    ...
    /**
     * Test moon phase calculations for 2025 against NASA data. NASA full moon dates for 2025: January 13, February 12,
     * March 14, April 13, May 12, June 11, July 10, August 9, September 7, October 7, November 5, December 4
     */
    @Test
    void testCalculateMoonPhases2025_MatchesNASAData() {
        ...
        LocalDate[] expectedFullMoons = {LocalDate.of(2025, 1, 13), LocalDate.of(2025, 2, 12),
                LocalDate.of(2025, 3, 14), LocalDate.of(2025, 4, 13), LocalDate.of(2025, 5, 12),
                LocalDate.of(2025, 6, 11), LocalDate.of(2025, 7, 10), LocalDate.of(2025, 8, 9),
                LocalDate.of(2025, 9, 7), LocalDate.of(2025, 10, 7), LocalDate.of(2025, 11, 5),
                LocalDate.of(2025, 12, 4)};

        // Filter full moon phases
        List<MoonPhaseData> fullMoons = phases.stream().filter(p -> p.phase == MoonPhase.FULL_MOON).toList();
        ...
        for (int i = 0; i < expectedFullMoons.length; i++) {
            ...
            assertTrue(daysDifference <= 1, String.format("Full moon %d: Expected %s, got %s (difference: %d days)",
    ...
    void testCalculateMoonPhases_PerformanceUnder100ms() {
        ...
        assertTrue(duration < 100, String.format("Moon phase calculation took %dms, should be < 100ms", duration));
    }

    /** Test moon phase value calculation (0.0 to 1.0 range). */
    @Test
    void testCalculateMoonPhaseValue_ValidRange() {
        LocalDate[] testDates = {LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 13), // Full moon
        ...
        for (LocalDate date : testDates) {
            ...
            assertTrue(phaseValue >= 0.0 && phaseValue <= 1.0,
                    String.format("Phase value for %s should be between 0.0 and 1.0, got %.4f", date, phaseValue));
        }
    }
    ...
    void testCalculateMoonIllumination_ValidData() {
        ...
        assertTrue(illumination.phase >= 0.0 && illumination.phase <= 1.0, "Phase should be between 0.0 and 1.0");
    ...
    void testIsMoonPhaseDay_DetectsFullMoon() {
        ...
        assertTrue(anyDetected,
                String.format(
                        "Should detect full moon within ±1 day of January 13, 2025. Detected:" + " %s:%s, %s:%s, %s:%s",
                        beforeFullMoon, astronomicalCalculationService.isMoonPhaseDay(beforeFullMoon), fullMoonDate,
                        astronomicalCalculationService.isMoonPhaseDay(fullMoonDate), afterFullMoon,
                        astronomicalCalculationService.isMoonPhaseDay(afterFullMoon)));
    }
    ...
    void testGetSunriseSunset_ValidData() {
        ...
        assertTrue(daylightMinutes > 840, // > 14 hours
                String.format("Summer solstice should have > 14 hours of daylight, got %d minutes"
                        + " (sunrise: %s, sunset: %s)", daylightMinutes, times.sunrise, times.sunset));
    }
    ...
    void testCalculateSeasonalEvents_Returns4Events() {
        ...
        SeasonalEvent springEquinox = events.stream().filter(e -> e.type == SeasonalEventType.SPRING_EQUINOX)
                .findFirst().orElseThrow();
        assertTrue(springEquinox.date.getMonthValue() == 3 && springEquinox.date.getDayOfMonth() >= 19
                && springEquinox.date.getDayOfMonth() <= 22, "Spring equinox should be around March 20");

        SeasonalEvent summerSolstice = events.stream().filter(e -> e.type == SeasonalEventType.SUMMER_SOLSTICE)
                .findFirst().orElseThrow();
        assertTrue(summerSolstice.date.getMonthValue() == 6 && summerSolstice.date.getDayOfMonth() >= 20
                && summerSolstice.date.getDayOfMonth() <= 22, "Summer solstice should be around June 21");

        SeasonalEvent autumnEquinox = events.stream().filter(e -> e.type == SeasonalEventType.AUTUMN_EQUINOX)
                .findFirst().orElseThrow();
        assertTrue(autumnEquinox.date.getMonthValue() == 9 && autumnEquinox.date.getDayOfMonth() >= 21
                && autumnEquinox.date.getDayOfMonth() <= 24, "Autumn equinox should be around September 22");

        SeasonalEvent winterSolstice = events.stream().filter(e -> e.type == SeasonalEventType.WINTER_SOLSTICE)
                .findFirst().orElseThrow();
        assertTrue(winterSolstice.date.getMonthValue() == 12 && winterSolstice.date.getDayOfMonth() >= 20
                && winterSolstice.date.getDayOfMonth() <= 23, "Winter solstice should be around December 21");
    }
    ...
    void testLeapYearHandling() {
        ...
        boolean hasFeb29 = hebrewDates.stream().anyMatch(d -> d.gregorianDate.equals(LocalDate.of(2024, 2, 29)));
        ...
        for (MoonPhaseData phase : phases) {
            switch (phase.phase) {
                ...
                case FIRST_QUARTER :
                    ...
                    break;
                case FULL_MOON :
                    assertTrue(Math.abs(phase.phaseValue - 0.5) < 0.1, "Full moon phase value should be near 0.5");
                    break;
                case LAST_QUARTER :
                    assertTrue(Math.abs(phase.phaseValue - 0.75) < 0.1, "Last quarter phase value should be near 0.75");
                    break;
                default :
                    break;
            }
